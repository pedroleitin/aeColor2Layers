...
When a project grows past 30–40 layers, scanning the timeline by name alone becomes slow. Color-coding helps — but doing it by hand (right-click layer → Label → pick from menu, for every layer) doesn't scale, and the choices stop matching what the layer actually looks like as the design evolves. Color→Layer reads the actual colors off the layer and assigns the closest label preset in one click.
...
4. `Window` menu → `Color→Layer` → the panel appears. Dock it next to Project / Effects / wherever.
...
- **v0.5** — UI refresh. Renamed the panel from **Color2Layer** to **Color→Layer** (window title, header, undo-group label, and the `.jsx` filename). Recolored the header accent from purple to orange. Added a thin rainbow strip across the top of the panel, painted band-by-band in a best-effort `onDraw` handler (ScriptUI has no native gradient brush). No color-matching behavior changed.
