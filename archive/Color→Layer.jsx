...
(function (thisObj) {
  ...
  function buildUI(thisObj) {
    ...
    // ===== Rainbow strip =====
    //
    // Thin decorative gradient bar across the very top of the panel. Drawn
    // band-by-band in a custom onDraw handler because ScriptUI has no native
    // gradient brush. Like the other graphics calls this is best-effort:
    // onDraw / the graphics API behave inconsistently across OS / AE builds,
    // so the whole thing is wrapped in try/catch and the panel works fine
    // even if nothing paints.
    var RAINBOW = [
      [0.90, 0.20, 0.20],   // red
    ...
    var splitBtn = win.add("button", undefined, "Split groups into layers");
    splitBtn.alignment = ["fill", "top"];
    splitBtn.maximumSize.width = 10000;
    splitBtn.preferredSize = [220, 34];
    splitBtn.helpTip = "Duplicate each selected shape layer and create one new layer per top-level shape group.";
    ...
    var statusText = statusGroup.add("statictext", undefined,
      "Select one or more shape layers and click a button.",
      { multiline: true });
